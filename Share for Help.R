...
### at the top of the list of names to be "fixed" with Chris, dammit.
### I've tried naming it, I've tried not naming it, I've tried changing it all using as_tibble- no luck!
### And for once, google has given me nothing useful.  Works great when they're vectors though.

d = tibble(c("Chris", "Jim", "Pam", "Leslie", "Ben", "Cameron", "Mitch", 
...
### !!!I couldn't get the fucking bind_col to work without errors, a metric ton
### of NAs, or an army of Carlas showing up in their own column instead of, 
### you know, BINDING THE FUCKING COLUMNS.  So many hours later, fuck it.  I'm out of ideas
drawn = tibble(Recipient = sample_n(d, 13, replace = FALSE))


### I've tried making Carla her own tibble, tried not, tried just adding her on, never works
carla = tibble(Recipient = "Carla")

# Add the sampled participants into the recipient vector

### Apparently, despite this code creating exactly what I need in ALL OTHER CASES, my arguments are 
### now lists containing data frames.  Which makes no sense, because a quick str() check
### Does not label either as a freaking list.  And when there's not an error, Carla has
### her own row filled with either her own name, or a single instance of her name followed by a column of NA
### but never binds with the row of drawn recipients.
r = bind_rows(carla, drawn)



#### So the above is the issue- I've manages bind_cols and functioning code everywhere else
#### Using the code I know so far.  Any help is appreciated.
...
### !!! and THESE gotdamn tibbles are bind_cols like they know wtf is up kyle.  So what the hell is up with Carla???
